...
A modern backend has to expose the *same* operations three ways: as an HTTP API
for the app, as [MCP](https://modelcontextprotocol.io) tools for assistants like
Claude and Cursor, and as tool definitions for AI agents. Done by hand, that is
the same surface described three times — three places to drift, three places to
keep typed.
...
HTTP route, the MCP tool, the agent tool and a fully-typed client. One source of
truth; the transports cannot drift.
...
- **Small.** ~4000 lines of source. No magic, no codegen, no build step in the
  consuming app.
