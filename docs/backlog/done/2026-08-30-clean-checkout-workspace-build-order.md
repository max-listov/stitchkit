...
The `0.69.0` release candidate passed the full local gate with existing ignored core/TUI `dist`
trees and nested Agent-starter `node_modules`, but exact-SHA CI runs failed during `bun run check`
on clean checkouts.
...
nonexistent package instead of using `@typescript/native-preview`'s local binary. Once installed,
the Agent starter then correctly exposed its next clean dependency: its file-linked
...
- [x] Build core and TUI packages in dependency order in the root prepare lifecycle before
      preparing nested starter trees.
...
- [x] A checkout with no core/TUI `dist` or Agent-starter `node_modules` can install and run
      `bun run check`.
...
- [x] Root [`package.json`](../../../package.json) builds core and TUI packages in dependency order
      during `prepare` before nested workspace preparation, and
      [`scripts/prepare-starter.ts`](../../../scripts/prepare-starter.ts) installs both locked
      starter trees, so every root gate dependency exists after a clean install.
...
- [x] Clean-artifact proof moved the previous ignored core/TUI `dist` trees and Agent-starter
      `node_modules` aside, ran `bun install --frozen-lockfile`, asserted generated core/TUI
      declarations and the local `tsgo` binary, then completed `bun run check` with every workspace
      green.
