...
function clientSource({ inject }) {
  ...
  return [
    ...
    "  if (connected) void client.request('ping', { n: 41 }, { timeoutMs: 5_000, onPhase: (observation) => requestPhases.push(observation.phase) }).then(finished.resolve, finished.reject);",
    '});',
    'client.connect();',
