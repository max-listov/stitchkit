...
/**
 * Root of the package this file was installed as: `dist/entrypoints/bin/upgrade-cli.js`
 * → the package, and `src/entrypoints/bin/upgrade-cli.ts` → the package when run
 * from source. The two trees are laid out alike, so one depth serves both.
 */
