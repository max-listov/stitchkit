...
export interface ToolPresentationConfig {
  ...
  /** A tool view's input defaults — what the model is told it gets by omitting a key. */
...
export function buildToolPresentationSchema(
  ...
  if (config.defaults) presentation = withToolDefaults(presentation, config.defaults);
...
/**
 * State a tool view's defaults in the document the model reads: the property
 * carries the tool's `default`, and a key the tool fills itself is no longer
 * required of the model. Without this the schema would advertise the HTTP
 * default and the model would pay for a key it never has to send. → ADR 0196.
 */
function withToolDefaults(
  ...
  const keys = Object.keys(defaults).filter((key) => isRecord(properties[key]));
  if (keys.length === 0) return schema;
  const nextProperties: ToolPresentationSchema = { ...properties };
  for (const key of keys) {
    const property = properties[key];
    // A copy: the document is deep-frozen below, and freezing the declared
    // value itself would freeze the application's own object with it.
    if (!isRecord(property)) continue;
    const value = structuredClone(defaults[key]);
    // Draft-07 ignores every keyword beside a `$ref`, so a default written next
    // to one would be invisible to the model; the reference moves into `allOf`.
    nextProperties[key] =
      typeof property.$ref === 'string'
        ? { allOf: [property], default: value }
        : { ...property, default: value };
  }
  const required = requiredKeys(schema).filter((key) => !keys.includes(key));
  const next: ToolPresentationSchema = { ...schema, properties: nextProperties };
  if (Array.isArray(schema.required)) next.required = required;
  return next;
}
