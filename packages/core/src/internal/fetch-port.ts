// WHATWG Fetch blocks these destination ports before any network request. An
// ephemeral listener can still receive one of them from the operating system,
// so a port-0 HTTP server must reject and rebind such an allocation before it
// exposes a URL intended for Fetch clients.
